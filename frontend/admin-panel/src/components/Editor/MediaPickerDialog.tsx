...
export const MediaPickerDialog: React.FC<MediaPickerDialogProps> = ({
  ...
  const getGalleryCover = (gallery: Content): string => {
    const media = (gallery.metadata as Record<string, unknown>)?.media;
    if (Array.isArray(media) && media.length > 0) {
      const firstImage = media[0] as Record<string, unknown>;
      const thumbnails = firstImage.thumbnails as Record<string, string> | undefined;
      return thumbnails?.small || thumbnails?.medium || (firstImage.s3_url as string) || '';
    }
    return '';
  };
                ...
                {filteredGalleries.map((gallery) => (
                  ...
                  >
                    <div className="aspect-square">
                      {(() => {
                        const coverUrl = gallery.featured_image || getGalleryCover(gallery);
                        return coverUrl ? (
                          <img
                            src={coverUrl}
                            alt={gallery.title}
